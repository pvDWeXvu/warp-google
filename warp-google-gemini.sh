...
test_connection() {
    ...
    # 测试 Google
    GOOGLE_TEST=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" https://www.google.com/ncr)
...
do_test_google() {
    ...
    RESULT=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" https://www.google.com/ncr)
    if [ "$RESULT" = "200" ]; then
